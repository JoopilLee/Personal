...
def plotFdist(n, title=None, fdist=None, fontprop=None):
    ...
    plt.bar(ind, freqs_np_vals*(ind+1))
...
elif str(platform.system()).lower().startswith('darwin'):
    #for Mac
...
pipeline = ptm.Pipeline(ptm.splitter.KoSentSplitter(),
                        ...
                        ptm.helper.POSFilter('NN*'), #1번 
                        # ptm.helper.POSFilter('VV*'), #2번
                        ptm.helper.SelectWordOnly(), #3번
                        ptm.helper.StopwordFilter(file='stopwords.txt'),
                        ptm.ngram.NGramTokenizer(min=2, ngramCount=3, concat=''),
                        ptm.counter.WordCounter())

#corpus = ptm.CorpusFromFile('sampleKor.txt')
#corpus = ptm.CorpusFromFile('../Untitled.csv')
corpus = ptm.CorpusFromFile('정조고.txt')
...
f = open("zipfs_result.txt", "w", encoding='utf8')
